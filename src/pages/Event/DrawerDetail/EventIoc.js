...
import BasicInfoIoc from './BasicInfoIoc';
import AttackVictimProfile from './AttackVictimProfile';
import AlertRelevanceInfo from './AlertRelevanceInfo';
/* eslint-disable no-underscore-dangle */

// const { TabPane } = Tabs;
@connect(({ iocDetail, loading }) => ({
  iocDetail,
  loading: loading.effects['iocDetail/fetchEventDetail'],
}))
class EventIoc extends Component {
  constructor(props) {
    super(props);
    this.state = {
      divChange: {
  ...
  componentDidMount() {
    ...
    if (query && query.id) {
      dispatch({
        type: 'iocDetail/fetchEventDetail',
        payload: {
          id: query.id,
  ...
  componentWillUnmount() {
    ...
    dispatch({
      type: 'iocDetail/saveWarningData',
  ...
  handleEvent = () => {
    ...
    dispatch({ type: 'eventFall/handleEvent', payload: { ids: [eventDetail._id] } })
      .then(() => {
      ...
      <div>
        ...
        <div>
          <div className={styles.titleDiv}>
            ...
            >
              {divChange.fall ? '收起' : '展开'}
            </a>
          </div>
          {divChange.fall && (
            <div className={styles.ctxDiv}>
              <AlertRelevanceInfo eventDetail={eventDetail} />
            </div>
          )}
        </div>
      </div>
