...
export default {
  ...
  effects: {
    *fetchAlarmList({ payload }, { call, put }) {
      ...
      const response = yield call(apiRequest, 'event/getSafeEventList', { ...payload, category_1: '入侵感知' });
    ...
    *fetchFilterCount({ payload }, { call, put }) {
      const response = yield call(apiRequest, 'event/getSafeEventFilterCount', { ...payload, category_1: '入侵感知' });
    ...
    //   const buckets = data && data.attackStage && Array.isArray(data.attackStage.buckets) ? data.attackStage.buckets : [];
    //   if (buckets.length === 0) {
    //     yield put({
    //       type: 'saveChartList',
    //       payload: {
