...
// import atpUpload from '../../../assets/image/atpUpload.png';
import AptUploadPage from './AptUploadPage';
...
class EventFile extends Component {
  constructor(props) {
              ...
              <div className={styles.tableAction}>
                ...
                {showOperation && (
                  <div className={styles.actionContent} style={actionStyle}>
                    {record.status === 'unhandled' && this.fileAuth === 'rw' && (
                      <Fragment>
                        <p
                          onClick={() => {
                    ...
                    <Link
                      target="_blank"
                      to={`/event/safeEvent/alarmFile?id=${record.id}&tsOldest=${record.tsOldest}&tsLatest=${record.tsLatest}`}
                    >
                      详情
                    </Link>
                  </div>
    ...
    this.fixTableRender = {
      ...
      md5: (text) => <div>{text}</div>,
      virusName: (text) => <div>{text}</div>,
      fileName: (text) => <div title={`${text}`}>{text}</div>,
      'affectedAssets.ip': (text, record) => {
        const { affectedAssets } = record;
        const arr = configSettings.ipsNoRepeat(affectedAssets);
        // console.log(222, 'arr===', arr);
        const popContent = (
          <div>
            {arr.map((item) => {
              if (item.assetName) {
      ...
      'dst.ip': (text, record) => {
        ...
        // console.log(8888, 'dstArr===', dstArr);
        const urls = configSettings.urlKey('ip');
        const popContent = (
          <div>
            {dstArr.map((item) => (
    ...
    this.localFilterKeys = localStorage.getItem(`fileFilterKeys`);
    this.localColumnKeys = localStorage.getItem(`fileColumnKeys`);
  ...
  componentWillMount = () => {
    ...
    // 本地上传页显示
    if (query && query.uploadPageShow && query.uploadPageShow === 'true') {
      this.setState({ uploadPageShow: true });
      delete query.uploadPageShow;
    }
    ...
    const tableScrollX = columnDefaultList.length * 90;
    ...
    queryKeys.forEach((key) => {
      ...
      if (obj) {
        ...
        // console.log('queryKeys',query[key]);
    ...
    if (query.startTime && query.endTime) {
      const diff = moment(parseInt(query.endTime, 10)).diff(
        moment(parseInt(query.startTime, 10)),
        'days'
      );
      otherObj = { startTime: parseInt(query.startTime, 10), endTime: parseInt(query.endTime, 10) };
      if (diff !== 1) {
  ...
  componentDidMount() {
    ...
    window.onscroll = () => {
      ...
      if (
        !this.state.uploadPageShow &&
        document.getElementsByClassName('ant-table-thead')[0] &&
        window.location.pathname === '/event/safeEvent/alarmFile' &&
        ...
        } else if (document.documentElement.scrollTop < this.state.h) {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    const { drawerID = '', drawerFLAG = 'apt', id: oldDrawerId } = drawerQuery;
    // 左侧抽屉详情
    // console.log(993, 'drawerID==', drawerID, 'drawerList.apt==', drawerList.apt);
    const curDrawerList = drawerList.apt[drawerID];
  ...
  detailShowClick = (record) => {
    ...
    drawerList.apt[record.id] = [record];
    dispatch({ type: 'eventOverview/saveFilterCount', payload: { drawerList } });
  ...
  drawerClose = () => {
    ...
    const curList = drawerList.apt[drawerQuery.drawerID];
    const len = curList.length;
    if (len > 1) {
      drawerList.apt[drawerQuery.drawerID] = curList.slice(0, len - 1);
    } else {
      delete drawerList.apt[drawerQuery.drawerID];
    }
    dispatch({ type: 'eventOverview/saveFilterCount', payload: { drawerList } });
  ...
  fieldManage = (list, type) => {
    ...
    if (type === 'column') {
      ...
      localStorage.setItem('fileColumnKeys', keyList.join(','));
      this.setState({
      ...
      localStorage.setItem('fileFilterKeys', keyList.join(','));
  ...
  fetchFilterCount = (newQuery) => {
    ...
    dispatch({
      type: 'eventFile/fetchFilterCount',
  ...
  mulitHandleEvent = () => {
    ...
    dispatch({ type: 'eventFile/handleEvent', payload: { ids } })
      .then(() => {
  ...
  mulitIgnoreEvent = () => {
    ...
    confirm({
      title: '忽略后不可恢复，确定忽略吗',
      onOk() {
        dispatch({ type: 'eventFile/ignoreEvent', payload: { ids } })
          .then(() => {
  ...
  dataHandler = (status, id) => {
    ...
    if (status === 'ignore') {
      confirm({
        title: '忽略后不可恢复，确定忽略吗',
        onOk() {
          dispatch({ type: 'eventFile/ignoreEvent', payload: data })
            .then(() => {
              message.success('已将该事件标识为“已忽略”状态,有延时，请稍后刷新页面查看');
              self.fetchAlarmData(query);
              self.fetchFilterCount(query);
      ...
      dispatch({ type: 'eventFile/handleEvent', payload: data })
        .then(() => {
          message.success('已将该事件标识为“已处理”状态，有延时，请稍后刷新页面查看');
          self.fetchAlarmData(query);
          self.fetchFilterCount(query);
  ...
  backEsTablePage = () => {
    window.location.href = '/event/safeEvent/alarmFile';
  };
  ...
  render() {
    const {
      ...
      tableLoading1,
      tableLoading2,
      eventFile: { alarmList, alarmChart },
      ...
      <div>
        {uploadPageShow ? (
          <div className="container">
            <AptUploadPage backEsTablePage={this.backEsTablePage} />
          </div>
        ) : (
          <div className="container">
            <div className={listPageStyle}>
              ...
              <Layout className={styles.contentBlock}>
                ...
                <Layout>
                  ...
                  <div className={styles.tableBlock} id={styles.tableBlockList}>
                    <div className={styles.handleBlock}>
                      <div className={styles.handleBar}>
                        ...
                        {this.upAuth.indexOf('r') > -1 && (
                          <Button
                            className="smallWhiteBtn"
                            onClick={() => {
                              window.location.href =
                                '/event/safeEvent/alarmFile?uploadPageShow=true';
                            }}
                          >
                            本地上传文件检测
                          </Button>
            ...
            >
              <div>
                ...
                {drawerQuery.category_1 === '异常文件感知' && (
                  <EventAptDrawer query={drawerQuery} />
                )}
              </div>
            </DrawerWidget>
          </div>
...
export default connect(({ global, msgNotify, eventOverview, eventFile, loading }) => ({
  hasVpc: global.hasVpc,
  ccsData: global.ccsData,
  eventFile,
  eventOverview,
  locationImgs: msgNotify.locationImgs,
  filterLoading: loading.effects['eventFile/fetchFilterCount'],
  tableLoading: loading.effects['eventFile/fetchAlarmList'],
  tableLoading1: loading.effects['eventFile/ignoreEvent'],
  tableLoading2: loading.effects['eventFile/handleEvent'],
}))(EventFile);
