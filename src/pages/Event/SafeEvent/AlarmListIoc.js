...
class EventFall extends Component {
  constructor(props) {
    super(props);
    this.iocAuth = getAuth('/event/safeEvent/alarmIoc');
    this.firstcolumns = [
      {
        title: '',
        width: 20,
              ...
              <div className={styles.tableAction}>
                ...
                {showOperation && (
                  <div className={styles.actionContent} style={actionStyle}>
                    {record.status === 'unhandled' && this.iocAuth === 'rw' && (
                      <Fragment>
                        <p
                          onClick={() => {
                    ...
                    <Link
                      target="_blank"
                      to={`/event/safeEvent/alarmIoc?id=${record.id}&tsOldest=${record.tsOldest}&tsLatest=${record.tsLatest}`}
                    >
                      详情
                    </Link>
                  </div>
    ...
    this.localFilterKeys = localStorage.getItem(`iocFilterKeys`);
    this.localColumnKeys = localStorage.getItem(`iocColumnKeys`);
  ...
  componentDidMount() {
    ...
    window.onscroll = () => {
      ...
      if (
        ...
        window.location.pathname === '/event/safeEvent/alarmIoc' &&
  ...
  componentWillReceiveProps(nextProps) {
    ...
    const { drawerID = '', drawerFLAG = 'ioc', id: oldDrawerId } = drawerQuery;
    // 左侧抽屉详情
    // console.log(993, 'drawerID==', drawerID, 'drawerList.ioc==', drawerList.ioc);
    const curDrawerList = drawerList.ioc[drawerID];
  ...
  detailShowClick = (record) => {
    ...
    drawerList.ioc[record.id] = [record];
    dispatch({ type: 'eventOverview/saveFilterCount', payload: { drawerList } });
  ...
  drawerClose = () => {
    ...
    const curList = drawerList.ioc[drawerQuery.drawerID];
    const len = curList.length;
    if (len > 1) {
      drawerList.ioc[drawerQuery.drawerID] = curList.slice(0, len - 1);
    } else {
      delete drawerList.ioc[drawerQuery.drawerID];
    }
    dispatch({ type: 'eventOverview/saveFilterCount', payload: { drawerList } });
  ...
  fieldManage = (list, type) => {
    ...
    if (type === 'column') {
      ...
      localStorage.setItem('iocColumnKeys', keyList.join(','));
      this.setState({
      ...
      localStorage.setItem('iocFilterKeys', keyList.join(','));
  ...
  fetchFilterCount = (newQuery) => {
    ...
    dispatch({
      type: 'eventFall/fetchFilterCount',
  ...
  selectRowOnchange = (selectedRowKeys, selectedRow) => {
  ...
  mulitHandleEvent = () => {
    ...
    dispatch({ type: 'eventFall/handleEvent', payload: { ids } })
      .then(() => {
  ...
  mulitIgnoreEvent = () => {
    ...
    confirm({
      title: '忽略后不可恢复，确定忽略吗',
      onOk() {
        dispatch({ type: 'eventFall/ignoreEvent', payload: { ids } })
          .then(() => {
  ...
  dataHandler = (status, id) => {
    ...
    if (status === 'ignore') {
      confirm({
        title: '忽略后不可恢复，确定忽略吗',
        onOk() {
          dispatch({ type: 'eventFall/ignoreEvent', payload: data })
            .then(() => {
              message.success('已将该事件标识为“已忽略”状态,有延时，请稍后刷新页面查看');
              self.fetchAlarmData(query);
              self.fetchFilterCount(query);
      ...
      dispatch({ type: 'eventFall/handleEvent', payload: data })
        .then(() => {
          message.success('已将该事件标识为“已处理”状态，有延时，请稍后刷新页面查看');
          self.fetchAlarmData(query);
          self.fetchFilterCount(query);
  ...
  render() {
    const {
      ...
      tableLoading1,
      tableLoading2,
      eventFall: { alarmList, alarmChart },
...
export default connect(({ global, msgNotify, eventOverview, eventFall, iocDetail, loading }) => ({
  eventFall,
  iocDetail,
  eventOverview,
  ...
  filterLoading: loading.effects['eventFall/fetchFilterCount'],
  tableLoading: loading.effects['eventFall/fetchAlarmList'],
  tableLoading1: loading.effects['eventFall/ignoreEvent'],
  tableLoading2: loading.effects['eventFall/handleEvent'],
}))(EventFall);
