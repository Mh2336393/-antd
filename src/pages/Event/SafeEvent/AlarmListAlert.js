...
class EventInvasion extends Component {
  constructor(props) {
    super(props);
              ...
              <div className={styles.tableAction}>
                ...
                {showOperation && (
                  <div className={styles.actionContent} style={actionStyle}>
                    {record.status === 'unhandled' && this.alertAuth === 'rw' && (
                      <Fragment>
                        <p
                          onClick={() => {
                    ...
                    <Link
                      target="_blank"
                      to={`/event/safeEvent/alarmAlert?id=${record.id}&tsOldest=${record.tsOldest}&tsLatest=${record.tsLatest}`}
                    >
                      详情
                    </Link>
                  </div>
          ...
          const popContent = (
            <div>
              ...
              <div style={{ width: '35px' }}>
                {/* <a
                  target="_blank"
                  href={`/event/safeEvent/${path}?id=${record.id}&tsOldest=${record.tsOldest}&tsLatest=${record.tsLatest}`}
                >
                  详情
                </a> */}
                <a
    ...
    this.localFilterKeys = localStorage.getItem(`alertFilterKeys`);
    this.localColumnKeys = localStorage.getItem(`alertColumnKeys`);
    ...
    // const { filterDefaultList, columnDefaultList } = setShowList(this.columnKey, this.filterKey, this.fixTableRender);
  ...
  componentWillMount = () => {
    ...
    queryKeys.forEach((key) => {
      ...
      if (obj) {
        ...
        paramFilterList.splice(paramFilterList.length - 1, 1);
  ...
  componentDidMount() {
    ...
    window.onscroll = () => {
      ...
      if (
        ...
        window.location.pathname === '/event/safeEvent/alarmAlert' &&
  ...
  componentWillReceiveProps(nextProps) {
    ...
    const { drawerID = '', drawerFLAG = 'alert', id: oldDrawerId } = drawerQuery;
    // 左侧抽屉详情
    // console.log(993, 'drawerID==', drawerID, 'drawerList.alert==', drawerList.alert);
    const curDrawerList = drawerList.alert[drawerID];
  ...
  detailShowClick = (record) => {
    ...
    drawerList.alert[record.id] = [record];
    dispatch({ type: 'eventOverview/saveFilterCount', payload: { drawerList } });
  ...
  drawerClose = () => {
    ...
    const curList = drawerList.alert[drawerQuery.drawerID];
    const len = curList.length;
    if (len > 1) {
      drawerList.alert[drawerQuery.drawerID] = curList.slice(0, len - 1);
    } else {
      delete drawerList.alert[drawerQuery.drawerID];
    }
    dispatch({ type: 'eventOverview/saveFilterCount', payload: { drawerList } });
    ...
    return (
      <Col span={21} className={styles.selectedList}>
        {Object.keys(filterObj).map((key) => {
          ...
          return (
            <Fragment key={key}>
              {filterObj[key].length > 0 && valueStr !== 'Invalid date' && (
  ...
  fieldManage = (list, type) => {
    ...
    if (type === 'column') {
      ...
      localStorage.setItem('alertColumnKeys', keyList.join(','));
      this.setState({
      ...
      localStorage.setItem('alertFilterKeys', keyList.join(','));
  ...
  //     newStatus = status.filter(item => item !== value);
  //   } else {
  //     newStatus = [value].concat(status);
  //   }
  //   const newQuery = Object.assign({}, query, { status: newStatus, page: 1 });
  //   this.setState(
  //     {
  //       query: newQuery,
  //       isAllCheck: false,
  ...
  fetchAlarmData = (newQuery) => {
    ...
    dispatch({
      type: 'eventInvasion/fetchAlarmList',
      payload: param,
    });
  };
  ...
  fetchFilterCount = (newQuery) => {
    ...
    dispatch({
      type: 'eventInvasion/fetchFilterCount',
  ...
  mulitHandleEvent = () => {
    ...
    dispatch({ type: 'eventInvasion/handleEvent', payload: { ids } })
      .then(() => {
  ...
  mulitIgnoreEvent = () => {
    ...
    confirm({
      title: '忽略后不可恢复，确定忽略吗',
      onOk() {
        dispatch({ type: 'eventInvasion/ignoreEvent', payload: { ids } })
          .then(() => {
  ...
  dataHandler = (status, id) => {
    ...
    if (status === 'ignore') {
      confirm({
        title: '忽略后不可恢复，确定忽略吗',
        onOk() {
          dispatch({ type: 'eventInvasion/ignoreEvent', payload: data })
            .then(() => {
              message.success('已将该事件标识为“已忽略”状态,有延时，请稍后刷新页面查看');
              self.fetchAlarmData(query);
              self.fetchFilterCount(query);
      ...
      dispatch({ type: 'eventInvasion/handleEvent', payload: data })
        .then(() => {
          message.success('已将该事件标识为“已处理”状态，有延时，请稍后刷新页面查看');
          self.fetchAlarmData(query);
          self.fetchFilterCount(query);
  ...
  render() {
    const {
      ...
      tableLoading1,
      tableLoading2,
      eventInvasion: { alarmList, alarmChart },
        ...
        <div className={listPageStyle}>
          <Row className={styles.titleBlock}>
            {/* <Col span={2} style={{ height: 44, lineHeight: '44px' }}>
                            <span className={styles.titleStyle}>告警列表</span>
                        </Col> */}
            {this.selctedItemsRender()}
          </Row>
...
export default connect(({ global, msgNotify, eventOverview, eventInvasion, loading }) => ({
  hasVpc: global.hasVpc,
  ccsData: global.ccsData,
  eventInvasion,
  eventOverview,
  locationImgs: msgNotify.locationImgs,
  filterLoading: loading.effects['eventInvasion/fetchFilterCount'],
  tableLoading: loading.effects['eventInvasion/fetchAlarmList'],
  tableLoading1: loading.effects['eventInvasion/ignoreEvent'],
  tableLoading2: loading.effects['eventInvasion/handleEvent'],
}))(EventInvasion);
