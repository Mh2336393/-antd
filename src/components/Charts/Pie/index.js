...
class Pie extends Component {
  ...
  render() {
    ...
    const defaultPadding = [12, 0, 12, 0];
      ...
      <div ref={this.handleRoot} className={pieClassName} style={style}>
        <ReactFitText maxFontSize={25}>
          <div className={styles.chart}>
            ...
            >
              {!!tooltip && (itemTpl ? <Tooltip title="x" itemTpl={itemTpl} /> : <Tooltip title="x" />)}
              <Coord type="theta" innerRadius={inner} />
              <Geom
                style={{ lineWidth, stroke: '#fff' }}
                tooltip={tooltip && geoTootip ? geoTootip : tooltipFormat}
                type="intervalStack"
                position="percent"
                color={['x', percent || percent === 0 ? formatColor : defaultColors]}
                selected={selected}
              >
                {hasLabel ? <Label content="x" /> : <span />}
              </Geom>
            </Chart>

            {(subTitle || total) && (
              <div className={styles.total} style={customTotalStyle}>
