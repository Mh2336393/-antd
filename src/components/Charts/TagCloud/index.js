  ...
  render() {
    ...
    return (
      <div className={classNames(styles.tagCloud, className)} style={{ width: '100%', height }} ref={this.saveRootRef}>
        {dv && (
          <Chart width={w} height={h} data={dv} padding={0} scale={{ x: { nice: false }, y: { nice: false } }}>
            <Coord reflect="y" />
            <Tooltip showTitle={false} />
            <Geom type="point" position="x*y" color="text" shape="cloud" tooltip="time*category" />
          </Chart>
        )}
      </div>
    );
